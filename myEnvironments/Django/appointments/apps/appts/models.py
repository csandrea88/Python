...
class ApptsManager(models.Manager):
    def Appts_validator(self, postData): 
        print "in appts manager"
        errors = []
    
        adddate = postData['adddate']
        addtime = postData['addtime']
        addtask = postData['addtask']
        #adddate_dateobj = datetime.strptime(adddate, '%Y-%m-%d')
        
        
        if len(adddate) < 1:
            errors.append("Appts: Date is required!")
        ...
        if len(addtime) < 1:
            errors.append("Appts: Time is required!")

        if len(addtask) < 1:
            errors.append("Appts: Task is required")
    ...
    def update_validator(self, postData): 
        print "in update validator"
        errors=[]

        updtdate = postData['date']
        updttime = postData['time']
        updttask = postData['task']
        updtstatus = postData['status']
        ...
        # print adddate_dateobj, date.today()
        # if adddate < date.today():
        #     errors.append("Appointment date must be current day or later!")
        # print "after add date len check" 
...
class Appts(models.Model):
    ...
    #Likes = models.ManyToManyField(Users, related_name="courses_Listed")
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
    objects = ApptsManager()
